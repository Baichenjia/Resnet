...
class ResNet50(tf.keras.Model):
    ...
    def __init__(self, data_format, name='',
        ...
        def conv_block(filters, stage, block, strides=(2, 2)):
            # 返回一个残差模块，尺度变小.
            return _ConvBlock(3, filters,   # kernel_size=3
                    stage=stage, block=block, data_format=data_format, strides=strides)

        def id_block(filters, stage, block):
            # 返回一个
            return _IdentityBlock(3, filters, 
                    stage=stage, block=block, data_format=data_format)
        ...
        if self.include_top:
            ...
            # 对每个feature_map进行average或者max
            reduction_indices = [1, 2] if data_format == 'channels_last' else [2, 3]
            reduction_indices = tf.constant(reduction_indices)
            if pooling == 'avg':
                self.global_pooling = functools.partial(
                    tf.reduce_mean, reduction_indices=reduction_indices, keep_dims=False)
            elif pooling == 'max':
                self.global_pooling = functools.partial(
